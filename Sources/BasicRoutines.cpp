 ...
 *              29 February 2016, 16:29 (last revised)
 *
 * @detail
...
void ParseCommandline(int argc, char *argv[], TParameters & parameters)
{
  ...
  if (!(n_flag && t_flag && i_flag  && m_flag) || 
      !(parameters.mode >= 0 && parameters.mode <= 2))
  {
    PrintUsageAndExit();
  }
  
  // implicit I/O intensity is 10
  if (!w_flag)
  {
    parameters.diskWriteIntensity = 10;
  }
...
void PrintUsageAndExit()
{
  ...
  fprintf(stderr,"  -w disk write intensity (implicit is 10, e.q. output every 10th iteration)\n");
